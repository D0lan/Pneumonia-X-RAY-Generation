...
# -------- Beta-VAE for Chest X-Ray (NORMAL + PNEUMONIA) ----------
...
version_name = "betaVAE_v1"

output_dir = f"outputs_{version_name}"
recon_dir  = f"{output_dir}/recon_{version_name}"
gen_dir    = f"{output_dir}/gen_{version_name}"
ckpt_dir   = f"{output_dir}/ckpt_{version_name}"
...
# Beta-VAE Loss
# -----------------------
def beta_vae_loss(recon_x, x, mu, logvar, beta=4.0, recon_weight=1.0):
    bce = nn.functional.binary_cross_entropy(recon_x, x, reduction="sum")
    kld = -0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp())
    loss = recon_weight * bce + beta * kld
    return loss, bce, kld
...
def train_and_test():
    ...
    for epoch in range(1, epochs+1):
        ...
        for imgs, labels in train_loader:
            ...
            # ---- USE BETA-VAE LOSS ----
            loss, _, _ = beta_vae_loss(recon, imgs, mu, logvar,
                                       beta=beta, recon_weight=recon_weight)
            loss.backward()
            optimizer.step()
            ...
            with torch.no_grad():
                ...
                for i in range(min(8, imgs.size(0))):
                    ...
                    utils.save_image(
                        ...
                        f"{recon_dir}/{folder}/epoch_{epoch}_idx_{i}.png"
                    )

            torch.save(model.state_dict(), f"{ckpt_dir}/beta_vae_epoch_{epoch}.pt")
    ...
    with torch.no_grad():
        for imgs, labels in test_loader:
            ...
            for i in range(min(8, imgs.size(0))):
                ...
                utils.save_image(
                    torch.cat([imgs[i], recon[i]], dim=2).cpu(),
                    f"{recon_dir}/{folder}/test_idx_{i}.png"
                )
            break
    ...
    with torch.no_grad():
        ...
        utils.save_image(samples.cpu(), f"{gen_dir}/normal/samples.png")

        # PNEUMONIA
        z = torch.randn(10, latent_dim).to(device)
        samples = model.decode(z)
        utils.save_image(samples.cpu(), f"{gen_dir}/pneumonia/samples.png")
